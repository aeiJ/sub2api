export default {
    upstreamChannels: {
      title: 'Upstream Accounts',
      description: 'Manage upstream account platforms, groups and API keys',
      searchPlaceholder: 'Search upstream account configs...',
      createButton: 'Create Upstream Account Config',
      createTitle: 'Create Upstream Account Config',
      editTitle: 'Edit Upstream Account Config',
      emptyTitle: 'No Upstream Account Configs',
      emptyDescription: 'Create an upstream account config to manage account-platform API keys',
      loadError: 'Failed to load upstream account configs',
      loadOneError: 'Failed to load upstream account config',
      groupsLoadError: 'Failed to load group options',
      createSuccess: 'Upstream account config created',
      updateSuccess: 'Upstream account config updated',
      deleteSuccess: 'Upstream account config deleted',
      createError: 'Failed to create upstream account config',
      updateError: 'Failed to update upstream account config',
      deleteError: 'Failed to delete upstream account config',
      deleteTitle: 'Delete Upstream Account Config',
      deleteConfirm: 'Are you sure you want to delete upstream account config "{name}"? This action cannot be undone.',
	      syncResultTitle: 'Sync Result: {name}',
	      syncSuccess: 'Sync completed',
	      syncFailed: 'Sync failed',
	      columns: {
	        name: 'Name',
	        platforms: 'Platforms',
	        keys: 'Keys',
	        details: 'Upstream Details',
	        updatedAt: 'Updated',
        actions: 'Actions'
	      },
	      list: {
	        platform: 'Platform',
	        group: 'Upstream / Local Group',
	        multipliers: 'Multipliers',
	        account: 'Account / Key',
	        supportedModels: 'Supported Models',
	        capacity: 'Load',
	        lastTest: 'Last Test',
	        speedTest: 'Speed Test',
	        localGroupRate: 'Local group',
	        upstreamGroupRate: 'Upstream group',
	        accountRate: 'Account',
	        keyPresent: 'Has Key',
	        keyMissing: 'No Key'
	      },
	      filters: {
	        allProviders: 'All Providers'
	      },
	      status: {
	        active: 'Active',
	        disabled: 'Disabled'
	      },
      providers: {
        anthropic: 'Anthropic',
        openai: 'OpenAI',
        gemini: 'Gemini',
        antigravity: 'Antigravity',
        grok: 'Grok',
        unknown: 'Unknown'
      },
      actions: {
        preview: 'Preview',
        sync: 'Sync',
        test: 'Test'
      },
      form: {
        name: 'Name',
        namePlaceholder: 'Enter upstream account config name',
        status: 'Status',
        description: 'Description',
        descriptionPlaceholder: 'Optional description',
        platforms: 'Platforms',
        platformsHint: 'A channel can include multiple account platforms, each syncing to matching accounts and groups.',
        noPlatforms: 'No platforms configured',
        poolCount: '{count} group configs',
        removePlatform: 'Remove platform',
        provider: 'Provider',
        displayName: 'Display Name',
        baseUrl: 'Base URL',
        groups: 'Group Config',
        groupsHint: 'Each group config syncs to a group and becomes the default group for the keys below.',
        addPool: 'Add Group',
        noPools: 'No group configs',
        removePool: 'Remove group',
        unnamedPool: 'Unnamed group',
	        poolName: 'Upstream Group Name',
	        groupName: 'Local Group Name',
	        groupNamePlaceholder: 'Select an existing local group or create a new one',
	        newGroupNamePlaceholder: 'Enter a new local group name',
        createGroupOption: 'Create group',
        createGroupLabel: 'Create group: {name}',
        createGroupDescription: 'Created during sync after save',
	        syncedGroup: 'Synced Local Group',
        groupMultiplier: 'Local Group Multiplier',
        upstreamGroupMultiplier: 'Upstream Group Multiplier',
        accountMultiplier: 'Account Multiplier',
        loadFactor: 'Load Factor',
        concurrency: 'Concurrency',
        keys: 'Account Key',
        keysHint: 'Each upstream group key maps to one account; blank existing keys are left unchanged.',
        accountKey: 'Mapped Account / API Key',
        accountKeyHint: 'One upstream group key syncs to one account and binds to the selected group above.',
        accountName: 'Account Name',
        addKey: 'Add Key',
        batchPlaceholder: 'Paste API keys, one per line. Optional format: name,key',
        addBatchKeys: 'Add Batch',
        noKeys: 'No API keys',
        keyName: 'Key Name',
        apiKey: 'API Key',
        apiKeyPlaceholder: 'Leave blank to keep the current key',
        syncedAccount: 'Synced Account',
        lastTest: 'Last Test',
        defaultPoolSuffix: 'Pool',
        defaultGroupSuffix: 'Group'
      },
	      preview: {
	        title: 'Sync Preview: {name}',
	        failed: 'Failed to load sync preview',
	        explain: 'This shows what will be written to Groups and Accounts. Preview and sync only manage configuration data, not routing, billing, or subscription logic.',
	        groups: 'Groups',
	        accounts: 'Accounts',
	        noChanges: 'No changes reported',
	        itemCount: '{count} item(s)',
	        syncedId: 'Linked ID #{id}',
	        warnings: 'Needs attention',
	        actions: {
	          createGroup: 'Create group',
	          updateGroup: 'Update group',
	          createAccount: 'Create account',
	          updateAccount: 'Update account',
	          skipAccount: 'Skip account',
	          error: 'Failed'
	        },
	        descriptions: {
	          createGroup: 'Adds a group during sync',
	          updateGroup: 'Updates an existing group during sync',
	          createAccount: 'Adds an account during sync',
	          updateAccount: 'Updates an existing account during sync',
	          skipAccount: 'Missing usable key, account will not be written',
	          error: 'Fix this before syncing',
	          other: 'Other sync action'
	        },
	        targets: {
	          group: 'Group Management',
	          account: 'Account Management'
	        },
	        warningMessages: {
	          missingApiKey: 'API key is missing, so the account will not be created or updated.',
	          missingOrInvalidApiKey: 'API key is missing or cannot be decrypted, so this account will be skipped.',
	          groupMissing: 'The previously linked group no longer exists. It will be treated as a new group.',
	          accountLookupFailed: 'Failed to locate the synced account by upstream key ID. Confirm Account Management after sync.'
	        }
	      },
	      test: {
	        title: 'Speed Test Result',
	        explain: 'The speed test checks each upstream group key mapped to an account and shows latency plus the upstream response.',
	        noResults: 'No test results',
	        channel: 'Channel',
	        keyId: 'Key #{id}',
	        groupName: 'Group: {name}',
	        accountId: 'Account ID #{id}',
	        latency: 'Latency',
	        model: 'Test Model',
	        supportedModels: 'Supported Models',
	        response: 'Response',
	        testedAt: 'Tested at',
	        success: 'Speed test completed',
	        failed: 'Speed test failed',
	        status: {
	          operational: 'Usable',
	          authError: 'Auth failed',
	          failed: 'Unavailable',
	          unknown: 'Unknown'
	        },
	        summary: {
	          operational: 'Usable keys',
	          authError: 'Auth failures',
	          failed: 'Unavailable',
	          unknown: 'Unknown'
	        },
	        messages: {
	          missingOrInvalidApiKey: 'API key is missing or cannot be decrypted'
	        }
	      },
      validation: {
        nameRequired: 'Please enter a channel name',
        platformRequired: 'Add at least one platform',
        platformDuplicate: '{platform} has already been added to this channel',
        platformInvalid: '{platform} requires display name and Base URL',
        poolInvalid: '{platform} has an incomplete group config',
        poolDuplicate: 'Group config "{pool}" is duplicated in the same platform',
        poolNumbersInvalid: 'Invalid numeric config in "{pool}"',
        keyNameRequired: 'Please enter a key name in "{pool}"',
        newKeyRequired: 'New keys in "{pool}" require an API key'
      }
    },

    upstreamAccountMonitor: {
      title: 'Upstream Monitor',
      description: 'Automatically monitors every current API key upstream account for availability and latency',
      searchPlaceholder: 'Search accounts...',
      runAll: 'Check All Upstream Accounts',
      enableAll: 'Enable Monitoring',
      disableAll: 'Disable Monitoring',
      enableMonitoring: 'Enable Monitoring',
      disableMonitoring: 'Disable Monitoring',
      fullEnableMonitoring: 'Enable All Monitoring',
      fullDisableMonitoring: 'Disable All Monitoring',
      cannotDisableSchedulable: 'Account scheduling is enabled, so monitoring cannot be disabled. Pause scheduling in account management first.',
      runOne: 'Check',
      checking: 'Checking',
      enabled: 'Enabled',
      disabled: 'Disabled',
      notEnabled: 'Monitoring off',
      notChecked: 'Not checked',
      success: 'Success',
      failed: 'Failed',
      autoRecover: 'Auto-recover account after successful check',
      autoRecoverHint: 'Clear error, rate-limit, and temporary unschedulable state after a successful check',
      noGroups: 'No groups',
      emptyTitle: 'No API key accounts',
      emptyDescription: 'There are no API key accounts available for automatic monitoring.',
      loadError: 'Failed to load account monitors',
      batchError: 'Batch operation failed',
      runSuccess: 'Check completed',
      runFailed: 'Check failed',
      runAllSuccess: 'Check completed: {success} succeeded / {failed} failed / {total} total',
      enableAllSuccess: 'Monitoring enabled: {enabled} enabled, {created} created, {skipped} skipped',
      disableAllSuccess: 'Monitoring disabled: {disabled} disabled, {skipped} skipped',
      windowTab: {
        '7d': '7 days',
        '15d': '15 days'
      },
      overall: {
        operational: 'OPERATIONAL',
        degraded: 'DEGRADED'
      },
      monitorStatus: {
        operational: 'Operational',
        degraded: 'Degraded',
        failed: 'Error'
      },
      sort: {
        default: 'Default Sort',
        availabilityAsc: 'Availability Low to High',
        availabilityDesc: 'Availability High to Low'
      },
      batch: {
        open: 'Batch Actions',
        title: 'Batch Actions',
        scopeAll: 'Scope: all API key accounts',
        scopeFiltered: 'Scope: accounts matching the current filters',
        checkTitle: 'Batch Check',
        checkDescription: 'Returns results account by account, so visible cards stop loading as soon as they finish.',
        runAll: 'Check all accounts',
        runFiltered: 'Check filtered accounts',
        progress: 'Completed {completed} / {total}, {success} succeeded, {failed} failed',
        monitorTitle: 'Enable/Disable Monitoring',
        monitorDescription: 'Enable or disable upstream account monitoring in bulk. Scheduled accounts are skipped when disabling.',
        enableAll: 'Enable all monitoring',
        enableFiltered: 'Enable filtered monitoring',
        disableAll: 'Disable all monitoring',
        disableFiltered: 'Disable filtered monitoring',
        intervalTitle: 'Batch Interval/Jitter Settings',
        intervalDescription: 'Update check interval and jitter in bulk without changing the monitoring switch.',
        intervalAll: 'Set all intervals',
        intervalFiltered: 'Set filtered intervals',
        intervalSuccess: 'Check interval updated: {updated} updated, {created} created, {skipped} skipped'
      },
      settings: {
        title: 'Monitor Settings',
        description: 'Configure the check model, interval, and random jitter for this upstream account.',
        primaryModel: 'Primary Model',
        primaryModelPlaceholder: 'Select a check model, or leave empty to use the default model',
        intervalMinutes: 'Check Interval (minutes)',
        jitterSeconds: 'Random Jitter (seconds)',
        modelLoadFailed: 'Failed to load supported models for this account',
        intervalInvalid: 'Check interval must be between 1 and 1440 minutes',
        jitterInvalid: 'Random jitter must be >= 0 and less than the interval in seconds',
        saveSuccess: 'Monitor settings saved',
        saveFailed: 'Failed to save monitor settings'
      },
      columns: {
        account: 'Account',
        platform: 'Platform',
        groups: 'Groups',
        accountStatus: 'Account Status',
        monitor: 'Monitor',
        latestResult: 'Latest Result',
        availability7d: '7d Availability',
        latency: 'Latency',
        lastRun: 'Last Check',
        nextRun: 'Next Check',
        actions: 'Actions'
      }
    },
}
